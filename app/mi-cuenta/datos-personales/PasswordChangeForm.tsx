...
export function PasswordChangeForm() {
  ...
  return <div className="account-password-fields">
    <label>Nueva contraseña<div className="account-password-entry"><input name="password" type={showPassword ? "text" : "password"} required minLength={MIN_PASSWORD_LENGTH} autoComplete="new-password" value={password} placeholder="Escribe o genera una contraseña" onChange={(event) => { setPassword(event.target.value); setCopyStatus(""); }} /><button className="button button-outline" type="button" onClick={generatePassword}>Generar clave</button><button className="button button-outline account-password-visibility-button" type="button" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? "Ocultar contraseña" : "Mostrar nueva contraseña"} aria-pressed={showPassword} title={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"}><VisibilityIcon visible={showPassword} /></button><button className="button button-outline account-password-copy-button" type="button" disabled={!password} onClick={copyPassword} aria-label="Copiar nueva contraseña" title="Copiar nueva contraseña"><CopyIcon /><span className="sr-only">Copiar nueva contraseña</span></button></div><small>{passwordRequirementText} Puedes crear una al azar y guardarla con seguridad.</small></label>
