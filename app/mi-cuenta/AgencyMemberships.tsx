      ...
      {agencyProfiles.map((agency) => {
        ...
        return <article className="agency-management-card" key={agency.id}>
          ...
          <form action={`/api/agencias/${agency.id}/invitaciones`} method="post" className="agency-invite-form">
            <label>Invitar una escort aprobada<select name="escort_profile_id" required defaultValue=""><option value="" disabled>Seleccionar perfil</option>{inviteOptions.map((escort) => <option key={escort.id} value={escort.id}>{escort.displayName} · {escort.city}</option>)}</select></label>
