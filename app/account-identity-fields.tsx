...
export function AccountIdentityFields({ values, birthDateReadOnly = false }: { values?: AccountIdentityValues; birthDateReadOnly?: boolean }) {
  return <div className="form-grid form-grid-two">
    ...
    <label>Tipo de documento (opcional)<select name="document_type" defaultValue={values?.documentType ?? "rut"}><option value="rut">RUT chileno</option><option value="foreign">Documento extranjero</option></select></label>
    <label>Número de documento (opcional)<input name="document_number" maxLength={40} autoComplete="off" defaultValue={values?.documentNumber ?? ""} placeholder="Ej. 12.345.678-5" /><small>Si lo ingresas, el RUT se valida; para documento extranjero usa el número tal como aparece en él.</small></label>
