...
export async function POST(request: NextRequest) {
  ...
  if (password.length < MIN_PASSWORD_LENGTH || !await resetPasswordWithToken(token, await hashPassword(password))) return NextResponse.redirect(new URL("/restablecer-clave?error=invalid", request.url), 303);
