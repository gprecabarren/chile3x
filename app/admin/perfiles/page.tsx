        ...
        <section className="admin-profile-list" aria-label="Anuncios para moderar">
          {pageRows.map((profile) => {
            ...
            return <article className="admin-profile-card" key={profile.id}>
              ...
              <dl className="admin-profile-card-owner"><div><dt>Cuenta propietaria</dt><dd><a href={"mailto:" + profile.ownerEmail}>{profile.ownerEmail}</a></dd></div><div><dt>Usuario de la cuenta</dt><dd><Link href={accountHref} prefetch={false}>{profile.ownerUsername ? "@" + profile.ownerUsername : "Ver detalles de la cuenta"}</Link></dd></div></dl>
