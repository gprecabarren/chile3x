...
export default async function AdminBugReportsPage({ searchParams }: { searchParams: Promise<{ estado?: string; notice?: string }> }) {
  ...
  return <AdminShell user={admin}><div className="admin-content">
    ...
    <nav className="admin-review-tabs">{Object.entries(labels).map(([key, label]) => <Link prefetch={false} className={selected === key ? "is-active" : undefined} href={`/admin/bugs?estado=${key}`} key={key}>{label}</Link>)}</nav>
