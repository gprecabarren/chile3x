...
export async function POST(request: NextRequest) {
  ...
  const [user] = await (await getDb()).select({ id: users.id, passwordHash: users.passwordHash }).from(users).where(eq(users.email, email)).limit(1);
  ...
  response.cookies.set({
    ...
    value: await createUserSession(user.id),
