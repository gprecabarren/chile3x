...
const statusLabel: Record<string, string> = {
  approved: "Publicado", draft: "Borrador", expired: "Vencido", paused: "Pausado", pending: "En revisión", rejected: "Requiere cambios",
};
...
const messages: Record<string, string> = {
  ...
  story_published: "La historia fue publicada y se ocultará automáticamente en 24 horas.",
  story_limit: "Ya alcanzaste el máximo de historias activas de ese tipo para esta publicación.",
  story_error: "No se pudo publicar la historia. Revisa el archivo o el estado de tu perfil.",
...
export default async function AccountHome({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const rows = await db.select({ id: profiles.id, slug: profiles.slug, displayName: profiles.displayName, type: profiles.type, status: profiles.status, city: profiles.city, region: profiles.region, updatedAt: profiles.updatedAt, pauseCount: listingPeriods.pauseCount, periodStatus: listingPeriods.status }).from(profiles)
  ...
  return <AccountShell user={user}><div className="account-content">
    <AccountHeading eyebrow="PANEL DE ANUNCIANTE" title="Tus perfiles" description="Guarda borradores, actualiza la información y envía cada aviso a revisión manual antes de publicarlo."><Link className="button button-primary" href="/mi-cuenta/nuevo-perfil">Crear perfil</Link></AccountHeading>
    {params.notice && <p className="account-success" role="status">{messages[params.notice] ?? messages.error}</p>}
    {rows.length === 0 ? <section className="account-empty"><h2>Aún no tienes perfiles</h2><p>Puedes crear un aviso de tipo escort, agencia o arriendo. Primero quedará como borrador y tú decides cuándo enviarlo a revisión.</p><Link className="button button-primary" href="/mi-cuenta/nuevo-perfil">Crear mi primer perfil</Link></section> : <div className="owner-profile-list">{rows.map((profile) => <article className="owner-profile-card" key={profile.id}><div><span className={`account-status account-status-${profile.status}`}>{statusLabel[profile.status]}</span><h2>{profile.displayName}</h2><p>{profile.type} · {profile.city}, {profile.region}</p></div><div className="owner-profile-actions">{profile.status === "approved" && <Link className="button button-public-preview" href={`/perfil/${profile.slug}`} target="_blank">Ver público</Link>}<Link className="button button-outline" href={`/mi-cuenta/${profile.id}/estadisticas`}>Estadísticas</Link><Link className="button button-outline" href={`/mi-cuenta/${profile.id}/editar`}>Editar</Link>{profile.status === "approved" && <Link className="button button-outline" href={`/mi-cuenta/${profile.id}/editar#historias`}>Historias</Link>}{(profile.status === "draft" || profile.status === "rejected") && <form action={`/api/perfiles/${profile.id}/enviar-revision`} method="post"><button className="button button-primary" type="submit">Enviar a revisión</button></form>}{profile.status === "approved" && profile.periodStatus === "active" && <form action={`/api/perfiles/${profile.id}/pausa`} method="post"><input type="hidden" name="action" value="pause" /><button className="button button-outline" type="submit">Pausar ({profile.pauseCount ?? 0}/2)</button></form>}{profile.status === "paused" && profile.periodStatus === "paused" && <form action={`/api/perfiles/${profile.id}/pausa`} method="post"><input type="hidden" name="action" value="resume" /><button className="button button-primary" type="submit">Solicitar reactivación</button></form>}</div></article>)}</div>}
