...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: post.canonicalUrl || `/noticias/${post.slug}` },
    robots: post.noindex ? { index: false, follow: true } : undefined,
    openGraph: { title: socialTitle, description: post.ogDescription || description, url: `/noticias/${post.slug}`, type: "article", siteName: "Chile3X", images: [image] },
...
export default async function NewsArticlePage({ params }: Props) {
  ...
  const schema = {
    ...
    publisher: { "@type": "Organization", name: "Chile3X", url: "https://chile3x.cl" },
    mainEntityOfPage: `https://chile3x.cl/noticias/${post.slug}`,
    ...(cover ? { image: `https://chile3x.cl/noticias/media/${cover.id}` } : {}),
  };
