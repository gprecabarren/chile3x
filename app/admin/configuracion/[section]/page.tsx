...
export default async function AdminSettingsSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return <AdminShell user={admin}><div className="admin-content">
    ...
    <form action="/api/admin/settings" method="post" className="admin-settings-form admin-settings-page-form"><input type="hidden" name="return_to" value={returnTo} />
      ...
      </div><div className="apple-secret-status" aria-label="Estado de secretos de Apple"><span className={appleSecrets.privateKey ? "is-ready" : "is-pending"}>APPLE_PRIVATE_KEY: {appleSecrets.privateKey ? "configurada" : "pendiente"}</span><span className={appleSecrets.encryptionKey ? "is-ready" : "is-pending"}>APPLE_TOKEN_ENCRYPTION_KEY: {appleSecrets.encryptionKey ? "configurada" : "pendiente"}</span></div><p className="telegram-implementation-note">La clave privada <code>.p8</code> y la clave aleatoria de cifrado nunca se pegan en este formulario ni se guardan en D1: se cargan como secretos de Cloudflare. Apple Developer Program es un servicio externo pagado; esta preparación no crea una suscripción ni genera cobros.</p><p><a className="text-link" href="https://developer.apple.com/help/account/capabilities/configure-sign-in-with-apple-for-the-web" target="_blank" rel="noreferrer">Abrir requisitos oficiales de Apple <span aria-hidden="true">→</span></a></p></section>}
      {name === "contacto" && <section className="admin-settings-section"><div><p>MEDIOS OFICIALES</p><h2>Contacto y redes</h2><span>Si un campo queda vacío, ese ícono no se muestra al público.</span></div><div className="admin-settings-grid"><label>WhatsApp del portal<input name="contact_whatsapp" inputMode="tel" maxLength={22} defaultValue={values.contact_whatsapp} placeholder="56912345678" /></label><label>Comunidad pública de Telegram<input name="contact_telegram" maxLength={180} defaultValue={values.contact_telegram} placeholder="@Chile3XComunidad o enlace t.me" /><small>Este mismo enlace se administra en el módulo Telegram y alimenta el botón del header y footer.</small></label><label>Instagram<input name="contact_instagram" maxLength={180} defaultValue={values.contact_instagram} placeholder="@chile3x o enlace de Instagram" /></label><label>Correo del portal<input name="contact_email" type="email" maxLength={180} defaultValue={values.contact_email} placeholder="contacto@chile3x.cl" /></label></div></section>}
