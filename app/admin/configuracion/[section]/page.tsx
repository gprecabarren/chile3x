...
export default async function AdminSettingsSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return <AdminShell user={admin}><div className="admin-content">
    ...
    <form action="/api/admin/settings" method="post" className="admin-settings-form admin-settings-page-form"><input type="hidden" name="return_to" value={returnTo} />
      ...
      {name === "google" && <section className="admin-settings-section"><div><p>INTEGRACIONES</p><h2>Google</h2><span>Pega solo los identificadores entregados por Google. Ambos ajustes son opcionales.</span></div><div className="admin-settings-grid"><label>Verificación de Search Console<input name="google_site_verification" maxLength={180} defaultValue={values.google_site_verification} placeholder="Código entregado por Google" /></label><label>Identificador de Google Analytics<input name="google_analytics_id" maxLength={20} defaultValue={values.google_analytics_id} placeholder="G-XXXXXXXXXX" /></label></div></section>}
