...
export function RegistrationPasswordFields() {
  ...
  return <div className="registration-password-fields"><label>Contraseña<div className="registration-password-entry"><input name="password" type={visible ? "text" : "password"} required minLength={MIN_PASSWORD_LENGTH} autoComplete="new-password" placeholder="Crea una contraseña de al menos 8 caracteres" /><button className="button button-outline account-password-visibility-button" type="button" onClick={() => setVisible((value) => !value)} aria-label={visible ? "Ocultar contraseñas" : "Mostrar contraseñas"} aria-pressed={visible} title={visible ? "Ocultar contraseñas" : "Mostrar contraseñas"}><VisibilityIcon visible={visible} /></button></div><small>{passwordRequirementText}</small></label><label>Repite tu contraseña<input name="password_confirmation" type={visible ? "text" : "password"} required minLength={MIN_PASSWORD_LENGTH} autoComplete="new-password" placeholder="Escribe nuevamente la misma contraseña" /><small>El botón de arriba muestra u oculta ambos campos.</small></label></div>;
}
