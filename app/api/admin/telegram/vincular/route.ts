...
export async function POST(request: NextRequest) {
  ...
  if (!admin) return wantsJson ? NextResponse.json({ error: "La sesión administrativa expiró." }, { status: 401 }) : new Response("No autorizado.", { status: 401 });
  ...
  if (!attempt) return wantsJson
    ...
    : NextResponse.redirect(new URL("/admin/telegram?notice=not_configured", request.url), 303);
