...
const notices: Record<string, string> = {
  ...
  bulletin_deleted: "Novedad eliminada de la web; el bot retirará también su mensaje sincronizado.",
...
export default async function AdminTelegramPage({ searchParams }: { searchParams: Promise<{ notice?: string; case_status?: string; case_action?: string; case_severity?: string; case_q?: string }> }) {
  ...
  return <AdminShell user={admin}><div className="admin-content telegram-admin-page">
    ...
    <section className="telegram-admin-section telegram-admin-guide">
      ...
      <details className="telegram-command-guide"><summary>Ver comandos disponibles</summary><dl><div><dt>Todos</dt><dd>/start, /help y /rules</dd></div><div><dt>Administradores vinculados</dt><dd>/status, /warn, /mute, /unmute, /ban, /unban, /novedad y /eliminar_novedad</dd></div></dl><p>Para eliminar una novedad desde Telegram, responde a su mensaje con /eliminar_novedad. Los comandos comprueban la identidad, el permiso correspondiente y la administración real del chat.</p></details>
    </section>
    ...
    <section className="telegram-admin-section">
      <header><div><p className="eyebrow">NOVEDADES</p><h2>Publicaciones sincronizadas</h2><p>Este flujo es independiente del blog. Desde Telegram, publica en el tema configurado o usa /novedad; solo se aceptan identidades administrativas vinculadas. Telegram no informa al bot cuando alguien borra manualmente un mensaje: para sincronizar la eliminación usa el panel o responde con /eliminar_novedad.</p></div><Link className="button button-outline" href="/novedades">Ver página pública</Link></header>
