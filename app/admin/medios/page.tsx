...
export default async function AdminMediaPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const [rows, usage, params] = await Promise.all([
    db.select({ media: profileMedia, profileName: profiles.displayName, profileSlug: profiles.slug, profileHandle: profiles.handle, profileStatus: profiles.status, ownerEmail: users.email }).from(profileMedia).innerJoin(profiles, eq(profileMedia.profileId, profiles.id)).innerJoin(users, eq(profiles.ownerId, users.id)).orderBy(desc(profileMedia.createdAt)),
